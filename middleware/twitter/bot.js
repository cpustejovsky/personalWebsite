...
module.exports = function likeAndRetweet(username) {
    ...
    const getTwitterData = async (params) => {
        let results = await T.get("/statuses/user_timeline", params, (err, data, response) => {
            let nameAndTweets = {
                name: "",
                tweets: []
            }
                        ...
                        if (err) throw err;
                            ...
                            T.post(`statuses/retweet/${id.id}`, function (err, response) {
                                if (err) throw err;
                                    ...
                                    nameAndTweets.tweets.push(response.full_text)
                                    if (!nameAndTweets.name) {
                                        nameAndTweets.name = response.user.name
                                    }
                                }
                            })
                        }
